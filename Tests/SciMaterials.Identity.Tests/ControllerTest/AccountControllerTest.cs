...
namespace SciMaterials.Identity.Tests.ControllerTest
{
    public class AccountControllerTest
    {
        ...
        public async Task LoginAsync_Return_StatusCode200_And_Result()
        {
            ...
            //Act
            var result          = await _AccountController.LoginUserAsync(request);
            var actionResultObj = Assert.IsType<Result<LoginUserResponse>>(result);
            var okResult        = Assert.IsAssignableFrom<Result<LoginUserResponse>>(actionResultObj);

            //Assert
            Assert.NotNull(okResult);
            Assert.True(okResult.Succeeded);
            Assert.Equal(expected_jwtSessionToken, okResult.Data.SessionToken);
        ...
        public async Task LogoutAsync_Return_StatusCode200_And_Result()
        {
            ...
            //Act
            var result          = await _AccountController.LogoutUserAsync();
        ...
        public async Task GetRefreshTokenAsync_Return_StatusCode200_And_Result()
        {
            ...
            var actionResultObj = Assert.IsType<Result<RefreshTokenResponse>>(result);
            var okResult        = Assert.IsAssignableFrom<Result<RefreshTokenResponse>>(actionResultObj);
            
            //Assert
            Assert.NotNull(okResult);
            Assert.True(okResult.Succeeded);
            Assert.Equal(expected_refresh_token, okResult.Data.RefreshToken);
